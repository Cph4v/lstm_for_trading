...
from tensorflow.keras.layers import Dense, Dropout, Conv1D, GlobalAveragePooling1D, Layer, LayerNormalization, Concatenate
...
class TransformerEncoder(Layer):
    ...
    def get_config(self):
        config = super().get_config().copy()
        config.update({'d_k': self.d_k, 'd_v': self.d_v, 'n_heads': self.n_heads, 'ff_dim': self.ff_dim, 'dropout_rate': self.dropout_rate})
        return config